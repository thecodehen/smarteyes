...
from util import query_light, upload_fileobj, set_user_state

def process_light(text, id, has_queried_date=False):
    if not has_queried_date:
        set_user_state(id, 'light')
        ...
        # sort by time
        values = query_light(start_time, end_time)
